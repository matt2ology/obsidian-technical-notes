...
**Step 2)** Make `row 1` negative of the same values and take the sum of `row 1` and
`row 2`; after, add `row 1` and `row 3`, and use the sum to replace `row 3`

> **Replacement**: Replace a row by itself plus any multiple of another row
> $\large -r_1\:+\:r_2\:\rightarrow\:r_2$ and $\large r_1\:+\:r_3\:\rightarrow\:r_3$
...
**Step 3)** Multiply `row 2` by $-4$ and add `row 3` with `row 3`; after, replace `row 3` the sum of `row 2`, that has been scaled by $-4$, and `row 3`
...
**Step 2)** Multiply `row 1` with $-1$, add `row 2`, and replace the sum with `row 2`
...
**Step 3)** Replace `row 2` with a $\frac{1}{4}$ scale down version of `row 2`

> **Scaling**: Multiply any row by a non-zero number $\frac{1}{4}r_2\:\rightarrow \:r_2$
